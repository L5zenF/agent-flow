    ...
    fn execute(input: ExecuteInput) -> Result<ExecuteOutput, ExecuteError> {
        ...
        if provider_id.is_empty() {
            return Err(ExecuteError::InvalidInput(
                "route-provider requires non-empty provider_id".to_string(),
            ));
        }
        ...
        let message_template = config
            .log_message
            .unwrap_or_else(|| "selected provider=${ctx.selection.provider_id}".to_string());
        ...
        Ok(ExecuteOutput {
            context_patch: Some(ContextPatch {
                ops: vec![
                    ...
                    ContextPatchOp::Remove("selection.model_id".to_string()),
                ],
            }),
            header_ops: Vec::new(),
            path_rewrite: None,
...
fn parse_config(config: Option<&JsonDocument>) -> Result<RouteProviderConfig, ExecuteError> {
    match config {
        ...
        None => Ok(RouteProviderConfig::default()),
    }
}
