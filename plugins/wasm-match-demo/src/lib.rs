...
impl Guest for Component {
    fn execute(input: ExecuteInput) -> Result<ExecuteOutput, ExecuteError> {
        ...
        let branch_id = config.branch_id.unwrap_or_else(|| "branch".to_string());
        ...
        if !is_supported_expression(&expr) {
            logs.push(LogEntry {
                level: LogLevel::Warn,
                message: format!(
                    "expression '{}' uses unsupported syntax; treating as no-match",
                    expr
                ),
            });
        }
        ...
        Ok(ExecuteOutput {
            ...
            header_ops: Vec::new(),
            path_rewrite: None,
            next_port: matched.then(|| NextPort {
                port: "match".to_string(),
            }),
            logs,
        })
    }
...
fn parse_config(config: Option<&JsonDocument>) -> Result<MatchConfig, ExecuteError> {
    match config {
...
fn is_supported_expression(expr: &str) -> bool {
    expr.split("&&")
        .map(str::trim)
        .all(|clause| evaluate_clause(clause, &empty_input()).is_some())
}
