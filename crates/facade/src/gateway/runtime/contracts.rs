...
pub(crate) fn execute_wasm_runtime_node(
    ...
    apply_runtime_output(node_id, plugin, &runtime_output, workflow_context, outgoing_headers, resolved_path);
...
fn build_runtime_execute_input(
    ...
    let granted_capabilities = node_config.granted_capabilities.iter().copied().collect::<HashSet<_>>();
    ...
    for capability in &manifest.capabilities {
        ...
        capability_declarations.push(RuntimeCapabilityDeclaration {
            ...
            required: true,
            scope: None,
            description: format!("{} capability required by plugin manifest", runtime_capability_name(&kind)),
        });
    ...
    for grant in &node_config.granted_capabilities {
        if !manifest.capabilities.iter().any(|capability| wasm_capability_from_manifest(capability) == *grant) {
            return Err(format!(
    ...
    Ok(RuntimeExecuteInput {
        ...
        node_config: RuntimeNodeConfig {
            manifest: RuntimePluginManifest {
                ...
                default_config_schema_hints_json: manifest.default_config_schema_hints.as_ref().map(toml_value_to_json),
                capabilities: capability_declarations,
            },
            grants: capability_grants,
            config_json: (!node_config.config.is_empty()).then(|| toml_value_to_json(&toml::Value::Table(node_config.config.clone()))),
        },
    })
}
...
pub(super) fn to_wit_execute_input(input: &RuntimeExecuteInput) -> ExecuteInput {
    ExecuteInput {
        ...
        request_headers: input.request_headers.iter().map(|header| RequestHeader {
            name: header.name.clone(),
            value: header.value.clone(),
        }).collect(),
        workflow_context: input.workflow_context.iter().map(|(key, value)| ContextEntry {
            key: key.clone(),
            value: value.clone(),
        }).collect(),
        ...
        node_config: NodeConfig {
            manifest: PluginManifest {
                ...
                default_config_schema_hints: input.node_config.manifest.default_config_schema_hints_json.as_ref().map(|json| JsonDocument { json: json.clone() }),
                capabilities: input.node_config.manifest.capabilities.iter().map(|capability| CapabilityDeclaration {
            ...
            grants: input.node_config.grants.iter().map(|grant| CapabilityGrant {
                kind: to_wit_capability_kind(&grant.kind),
                allowed: grant.allowed,
                scope: grant.scope.clone(),
            }).collect(),
            config: input.node_config.config_json.as_ref().map(|json| JsonDocument { json: json.clone() }),
        },
    }
}
...
pub(super) fn from_wit_execute_output(output: ExecuteOutput) -> RuntimeExecuteOutput {
    RuntimeExecuteOutput {
        context_ops: output.context_patch.map(|patch| {
            patch.ops.into_iter().map(|op| match op {
                ContextPatchOp::Set(entry) => RuntimeContextPatchOp::Set { key: entry.key, value: entry.value },
                ContextPatchOp::Remove(key) => RuntimeContextPatchOp::Remove { key },
            }).collect()
        }).unwrap_or_default(),
        header_ops: output.header_ops.into_iter().map(|op| match op {
            HeaderOp::Set(header) => RuntimeHeaderOp::Set { name: header.name, value: header.value },
            HeaderOp::Append(header) => RuntimeHeaderOp::Append { name: header.name, value: header.value },
...
fn capability_scope(kind: WasmCapability, node_config: &WasmPluginNodeConfig) -> Option<String> {
    match kind {
        ...
        WasmCapability::Fs => Some(format!("read={};write={}", node_config.read_dirs.join(","), node_config.write_dirs.join(","))),
...
fn escape_json_string(value: &str) -> String {
    value.chars().flat_map(|ch| match ch {
