...
use crate::config::{
    resolve_workflow_path, runtime_state_from_config, save_config_atomic, save_workflow_file_atomic,
    WorkflowFileConfig,
};
...
pub async fn create_workflow(
    ...
    let next_runtime_state = match runtime_state_from_config(&state.config_path, next_config.clone()) {
...
pub async fn put_workflow(
    ...
    let mut runtime_state = state.runtime_state.write().await;
    let workflow = require_workflow(&to_application_workflow_entries(&runtime_state.config.workflows), &id)
        .map_err(workflow_admin_error)
    ...
    let next_runtime_state = match runtime_state_from_config(&state.config_path, runtime_state.config.clone()) {
        Ok(next_runtime_state) => next_runtime_state,
        Err(error) => {
...
pub async fn activate_workflow(
    ...
    let next_runtime_state =
        runtime_state_from_config(&state.config_path, next_config.clone()).map_err(invalid_request)?;
